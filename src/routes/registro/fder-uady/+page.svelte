...
<script lang="ts">
    ...
    let matriculaValues: string[] = $state(Array(2));
    ...
    function onChangeMatricula(ev: Event, index: number) {
        let input = ev.target as HTMLInputElement;
        matriculaValues[index] = input.value.replace(/[^\d]/gi, "");

        if (!validateNumInput(input.value, 10000000, 99999999)) {
...
<!-- Título -->
<TituloRegistro text="Registro de delegaciones FDER UADY"></TituloRegistro>
...
<form bind:this={formDelegaciones} method="POST" action="https://api.smmun.com/registro/delegaciones" name="registro-delegaciones" enctype="multipart/form-data" class="col g-3" novalidate onsubmit={onSubmitForm}>
    <div class="mb-3">
        <p>
            <strong>NOTA:</strong> este registro es exclusivo para quienes forman parte de la Delegación Oficial de la Facultad de Derecho UADY. De no ser su caso, puede encontrar el registro regular <a href="/registro/delegaciones">aquí</a>.
        </p>
    </div>
    ...
    <input type="hidden" name="delegacion_oficial" value="si">
    <input type="hidden" name="nombre_delegacion_oficial" value="Facultad de Derecho UADY">
    <input type="hidden" name="responsable_delegacion_oficial" value="Pendiente">
    ...
    {#each { length: 2 }, i}
        {#if i == 0 || modalidadValue == "pareja" || !loadedCookies} <!-- No mostrar la segunda sección si no se eligió codelegación -->
            ...
            <div class="mb-3">
                ...
                <input type="hidden" name="escolaridad_{i}" value="Universidad">
                <input type="hidden" name="escuela_{i}" value="Facultad de Derecho UADY">

                <label for="matricula_{i}" class="form-label">Datos UADY <span style="color: red;">*</span></label>
                <div class="row g-3">
                    <div class="col">
                        <input oninput={(ev) => onChangeMatricula(ev, i)} bind:value={matriculaValues[i]} class="form-control" required autocomplete="off" name="matricula_{i}" placeholder="Matrícula" type="text" minlength="8" maxlength="8" inputmode="numeric" class:is-invalid={!validateNumInput(matriculaValues[i], 10000000, 99999999)}>
                    <div class="invalid-feedback">La matrícula no es válida.</div>
                    </div>
                    <div class="col">
                        <input oninput={onChangeSaveCookie} class="form-control" required autocomplete="off" name="grado_seccion_{i}" placeholder="Grado y sección" type="text" maxlength="150">
                        <div class="invalid-feedback">Debes llenar este campo.</div>
                    </div>
                </div>
    ...
    <div class="mb-3">
        ...
        <p>
            {#if modalidadValue == "pareja"}
                <strong>Costo de inscripción (precio preferencial FDER):</strong> $440 MXN ($220 MXN por persona)
            {:else}
                <strong>Costo de inscripción (precio preferencial FDER):</strong> $220 MXN
            {/if}
        </p>

        <p>
            <strong>Concepto:</strong> Nombre y apellido FDER
        </p>
