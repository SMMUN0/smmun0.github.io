        ...
        try {
            ...
            if (delegacionesEnDom > num) {
                ...
                for (let i = delegacionesEnDom; i < num; i++) {
                    ...
                    // Validar inputs
                    validarCorreos(delegacion.getElementsByClassName("input-correo") as HTMLCollectionOf<HTMLInputElement>);
                    validarCelulares(delegacion.getElementsByClassName("input-celular") as HTMLCollectionOf<HTMLInputElement>);
                    validarEdades(delegacion.getElementsByClassName("input-edad") as HTMLCollectionOf<HTMLInputElement>);

                    // Manejar escolaridad
                    let escuelaInput = delegacion.querySelector(`#input-escuela-d${i}`) as HTMLInputElement;
                    let escuelaLabel = delegacion.querySelector(`#label-escuela-d${i}`) as HTMLLabelElement;
                    (delegacion.querySelector(`#select-escolaridad-d${i}`) as HTMLSelectElement).addEventListener("change", function() {
